...
export class UserService {
  ...
  async update ({ name, email, bio }: UpdateUserInput, authUser: User): Promise<UpdateUserOutput> {
    try {
      const { id } = authUser;
  ...
  async changePassword ({ oldPassword, newPassword }: ChangePasswordInput, authUser: User): Promise<changePasswordOutput> {
    try {
      const { id } = authUser;

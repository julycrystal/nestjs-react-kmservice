...
import { MY_ORDER_LIST_QUERY } from "../../../../graphql/order.graphql";
import { MyOrderQuery, MyOrderQueryVariables, MyOrderQuery_myOrders_data_orders } from "../../../../__generated__/MyOrderQuery";
...
const MyOrders = () => {
    ...
    const [orders, setOrders] = useState<MyOrderQuery_myOrders_data_orders[]>([]);
    const orderPerPage = 10;

    const [fetchOrders] = useLazyQuery<MyOrderQuery, MyOrderQueryVariables>(
        MY_ORDER_LIST_QUERY,
        {
            fetchPolicy: "no-cache",
            onCompleted: ({ myOrders }) => {
                const { ok, data } = myOrders;
                if (ok && data?.orders) {
